...
This server launches Playwright against a FIXED on-disk Chrome profile
(``config.CHROME_PROFILE``). That profile is where the operator's signed-in
LinkedIn session lives, which is the only reason he does not have to sign in
again on every call. Two processes pointed at the SAME user-data dir corrupt
it -- Chromium is not safe for concurrent use of one profile directory -- and
a corrupted profile means the session is gone and he has to sign in again.
An outage from exactly this cost 37 minutes on a sibling server, which is why
the lock is mandatory rather than advisory.

The in-process lock in ``browser.py`` only serialises calls WITHIN one
process. It does nothing across processes. This module adds the missing
...
* ``acquire()`` writes ``<profile>.lock`` containing this process's PID + a
  monotonic-ish start marker. Before writing it inspects any existing lock:
    ...
    - our OWN PID (re-entrant: the idle-close timer stops the browser and
      the next tool call starts it again in the same process) -> succeed,
      refresh the lock.
...
class ProfileLockedError(RuntimeError):
    ...
    stop. Refusing to launch here is the whole point -- corrupting the profile
    is far costlier than failing loudly.
    """
...
def acquire() -> None:
    ...
    Re-entrant for the current process (the idle-close timer stops the
    browser and a later call restarts it in the same PID). Reclaims stale
    locks left by crashed
    ...
    if existing_pid is not None:
        if existing_pid == os.getpid():
            # Same process re-acquiring (e.g. after an idle close). Refresh + go.
