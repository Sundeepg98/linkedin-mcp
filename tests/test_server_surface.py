"""The tool surface: fifty-one tools, thirty-nine of which do not write.
...
EXPECTED_TOOLS = {
    ...
    # THE FIFTIETH AND FIFTY-FIRST, 2026-09-23 (lane L3), both READS on the
    # jobs surface. ``linkedin_recent_job_searches`` reads the jobs home,
    # /jobs/jam/, admitted 2026-09-20 with nothing behind it, and banks census
    # J 18; ``linkedin_tracked_job_proximity`` joins the tracker to the posting
    # page's proximity read and banks J 57. Neither adds an address, a
    # sanctioned mutation or a press.
...
async def test_the_surface_is_exactly_the_fiftyone_tools(tools):
    """RENAMED AGAIN 2026-09-23 from ``..._fortynine_tools``, because two reads
    arrived in one commit: ``linkedin_recent_job_searches`` (census J 18) and
    ``linkedin_tracked_job_proximity`` (census J 57).
    ...
    **THAT SENTENCE READ "THIRTY-SIX NAMES OVER THIRTY-FIVE" UNTIL 2026-09-05
    ...
    # FIFTY-ONE FROM 2026-09-23 (lane L3): linkedin_recent_job_searches and
    # linkedin_tracked_job_proximity, two READS on addresses the boundary
    # already admitted. The NAME moved with them again.
    assert len(tools) == 51
    ...
    # THIRTY-NINE FROM 2026-09-23: linkedin_recent_job_searches and
    # linkedin_tracked_job_proximity are BOTH READS, and the write side is
    # BYTE-IDENTICAL across them: no entry in readonly.SANCTIONED_MUTATIONS,
    # no new address, no press.
