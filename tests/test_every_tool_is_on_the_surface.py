...
def test_both_rules_reject_the_registry_that_was_actually_measured():
    ...
    # FIFTY FROM 2026-09-23 (lane L3): linkedin_recent_job_searches, a READ,
    # banking census row J 18 as COVERED-UNFIRED in the same commit. It opens
    ...
    # in the merge commit.
    # FIFTY-FOUR FROM 2026-09-24, at the merge of lane L5: two READS
    # (linkedin_list_conversations, linkedin_open_thread) and a WRITE
    # (linkedin_send_reply), moved with every other count site in one commit.
    assert len(_tool_names()) == 54
