...
NOT_A_CORRECTION: dict[tuple[str, str], str] = {
    ...
    ("2026-09-21-the-four-loose-rows.md", "2026-09-20-the-contingent-writeoffs.md"): (
        "THE CORRECTION VOCABULARY IS QUOTED FROM THE CITED DOCUMENT, in a "
        "table reporting what four earlier documents each concluded about "
        "census row `J 57`. The matched words -- *overturns*, *FALSE NOW* -- "
        "are that document's own verdict on a WRITE-OFF, not this document's "
        "verdict on it. "
        "**THE CORRECTION IN THIS AREA IS ALREADY DECLARED AND IT IS A "
        "DIFFERENT PAIR:** `2026-09-20-the-contingent-writeoffs.md` carries a "
        "CORRECTED BY: marker naming `2026-09-20-the-three-held-defects.md`, "
        "which carries the matching CORRECTS:. This wave adds nothing to that "
        "pair and disturbs neither end of it. "
        "**AND THE STATE DID NOT MOVE:** `J 57` was GAP before this wave and "
        "is GAP after it -- all four documents in the table agree on the "
        "state and differ only on the BLOCKER, whose re-file stays deferred "
        "on the decision point three-held-defects section 1.4 named. "
        "Declaring a CORRECTS: pair would send a reader looking for a "
        "reversal this wave did not make."
    ),
    ("2026-09-21-the-four-loose-rows.md", "2026-09-20-the-three-held-defects.md"): (
        "THE SAME CHRONOLOGY TABLE, ONE ROW DOWN, and this citation is the "
        "opposite of a correction: it DEFERS to the cited document twice. It "
        "adopts that document's verdict that `J 57` loses its `SKILL` tag, "
        "and it adopts its section 1.4 reasoning for why the blocker re-file "
        "onto `PROXIMITY-NOT-PARSED` must NOT be performed -- that the name "
        "exists nowhere else in the repository and creating it breaks three "
        "pinned assertions. This wave re-verified that deferral still holds "
        "at HEAD and left it in place. "
        "A `CORRECTED BY:` marker on that document would read as closure of "
        "an explicitly OPEN decision, which is worse than silence."
    ),
    ("2026-09-21-the-four-loose-rows.md", "2026-09-21-the-jobs-direction.md"): (
        "THE THIRD ROW OF THE SAME TABLE, and the weakest candidate of the "
        "three: the cited document neither corrects nor is corrected. The "
        "row records that it RESTATES the already-settled position and "
        "classes `J 57` as *dependent rather than independent*. The "
        "correction vocabulary the scan matched belongs to the table's other "
        "rows, two and three lines away, which is the proximity limit this "
        "file exists to force a human through."
    ),

    # 2026-09-21, the same wave. AGREEMENT, NOT REVERSAL -- and this is the
    # entry worth reading, because the two documents reach the same finding
    # and only one of them acted on it.
    ("2026-09-21-the-four-loose-rows.md", "2026-09-21-what-the-browser-said.md"): (
        "THE CITED SECTION IS RIGHT AND THIS WAVE SAYS SO. Section 6.1 "
        "measured that `shape.invitation_badge`'s stated premise is false -- "
        "`census_shape` is a character and length gate, so a short plain name "
        "passes it unchanged -- and then DELIBERATELY DECLINED to repair it, "
        "on the argument that the two honest repairs are incompatible and "
        "*half of it is worse than none*. "
        "This wave drove the same measurement independently, reproduced it, "
        "and then TOOK the first of the two repairs that section itself "
        "named: correct the claim, keep publishing the label, and pin both "
        "halves with a test so the stronger claim cannot drift back. "
        "**ACTING ON A DOCUMENT'S OWN RECOMMENDATION IS NOT CORRECTING IT.** "
        "A `CORRECTED BY:` marker would tell a reader that section 6.1 was "
        "wrong. It was right, its reasoning is what this wave executed, and "
        "the fork it declined to close -- publish the label, or publish its "
        "shape -- is still open and recorded as open at both ends."
    ),

    # 2026-09-21, the FOUR-LOOSE-ROWS wave again, and the SAME TWO DERIVED
    # VIEWS IN ONE BREATH that the what-the-browser-said wave hit below. One
    # reason, told twice, because the scan keys on a basename.
    ("2026-09-21-the-four-loose-rows.md", "INDEX.md"): (
        "THE MATCHED VOCABULARY IS THIS DOCUMENT'S OWN CHANGE-LIST, NOT A "
        "VERDICT ON EITHER CITED FILE. The line is a bullet in section 5 "
        "recording that `_audit/INDEX.md` and `_audit/RULINGS.md` were "
        "REGENERATED by `build_audit_index.py --write` and "
        "`build_rulings_index.py --write` after the impact gate went red on "
        "their derived-file tests. The correction words the scan matched "
        "belong to the NEIGHBOURING bullets, which describe what this wave "
        "did to two DOCSTRINGS and to one allowlist. "
        "**REGENERATING A DERIVED VIEW IS NOT CORRECTING IT** -- the file had "
        "no claim of its own to be wrong about; it is computed from the "
        "corpus, and this wave added a document to the corpus. Declaring a "
        "CORRECTS: pair would put a `CORRECTED BY:` marker inside a generated "
        "file, where the next `--write` would silently delete it."
    ),
    ("2026-09-21-the-four-loose-rows.md", "RULINGS.md"): (
        "THE SAME LINE AND THE SAME REASON as the INDEX.md entry above; the "
        "two derived views are named in one breath and the scan keys on a "
        "basename, so splitting the reason would be a lie about how many "
        "facts there are. "
        "Worth one addition specific to this file: the register's count is "
        "UNCHANGED at 37 across this wave, because regenerating it registered "
        "no new ruling. This wave APPLIED "
        "`ERROR-MESSAGE-RULED-AT-THE-RAISE` to a field and recorded that no "
        "ruling reaches another; neither act declares a ruling, and the "
        "register would be wrong to gain an entry for either."
    ),

    # 2026-09-21, the same wave. A CITATION OF A CENSUS FILE AS THE THING
    # BEING MEASURED, not as a document being contradicted.
    ("2026-09-21-the-four-loose-rows.md", "jobs.md"): (
        "THE CITED FILE IS THE MEASUREMENT'S SUBJECT, AND THE WRONGNESS "
        "BELONGS TO A POINTER AT IT RATHER THAN TO ANYTHING IT SAYS. The "
        "line reports that two blocker-assignment rows, `J 18` and `J 19`, "
        "both cite `_audit/_census/jobs.md` line 428, that at most one could "
        "ever have been right, and that in fact neither is -- line 428 is row "
        "143. **The defect is in "
        "`_audit/_census/blocker-assignments.tsv`'s locator column, not in "
        "`jobs.md`**, which is simply the file those locators fail to point "
        "into after the document grew. "
        "No census row's state, text or reason was changed by this wave -- "
        "704 stated rows before and after -- so there is nothing in that file "
        "for a CORRECTED BY: marker to point at. The repair the measurement "
        "argues for is a locator that resolves to a symbol, and it is "
        "recorded as an open residual rather than performed."
    ),
    ...
    ("profile.md", "2026-09-23-live-lane-session-1.md"): (
        "TABLE-ROW PROXIMITY, from ELEVEN citing lines, every matched word in "
        "a NEIGHBOURING row. The session document is cited by rows `A8`, `A11`, "
        "`A13`, `A17`, `A19`, `A21` (each 'NOT SAVED', recording why the "
        "profile-edit proof did not save), `A25`, `L1`, `L8` (each 'CAPTURED', "
        "state unchanged), `G6` (banked on the session's fire) and `O3` (half "
        "proven, state unchanged). Read window by window: `A13`, `A17` and "
        "`A21` sit beside rows `A14`, `A15` and `A22`, whose own paragraph "
        "reads 'the blocker's premise is now wrong: the controls are read' -- "
        "about those rows' blockers, written before this session; `G6` sits "
        "above `G7`, whose 2026-09-05 paragraph reads '\"no tool, no reason\" "
        "is now half false'; and `O3` sits two rows above `O5` ('ALL THREE "
        "CLAUSES ARE FALSE AND THE ROW MOVES ON THE CORRECTION', about badge "
        "creation). No citing "
        "line withdraws anything from the session document. **WHAT WOULD MAKE "
        "THIS ENTRY WRONG:** any of those rows asserting that something IN the "
        "session document is false while leaving it untouched."
    ),
    ("network.md", "2026-09-23-live-lane-session-1.md"): (
        "TABLE-ROW PROXIMITY, AND ONE ROW'S OWN HISTORY. The citing line is "
        "row `134`, whose 2026-09-23 23:08 paragraph banks it on the session's "
        "fire and cites the session document as its evidence. Read window by "
        "window: rows `132`, `133`, `135` and `136` each carry their own "
        "'corrected' / 'correction' / 'false' / 'wrong' from earlier waves; "
        "and row `134`'s older paragraphs carry 'correction' and 'false' "
        "about its own earlier blockers -- including this lane's earlier wave "
        "recording it NOT PROVEN, which was true under the rulings of that "
        "hour and moved because a ruling (VIEW-SWITCH-PRESS-RESTORED) and a "
        "build arrived, not because it was wrong. No citing line withdraws "
        "anything from the session document. **WHAT WOULD MAKE THIS ENTRY "
        "WRONG:** row `134` asserting that something IN the session document "
    ...
    ("messaging-and-content.md", "2026-09-23-live-lane-session-1.md"): (
        ...
        "ONE table line, where the matched `false` is its own 2026-09-21 "
        "paragraph -- 'nothing this census publishes had to be false', about "
        "the direction cell -- two days older than this session's NEEDS-TARGET "
        "paragraph on the same line. No citing line withdraws anything from "
        "the session document. **WHAT WOULD MAKE THIS ENTRY WRONG:** any of "
        "those rows asserting that something IN the session document is false "
    ...
    ("profile.md", "2026-09-24-lane-l7-profile-writes.md"): (
        ...
        "OR THE ROW'S OWN. Forty-one rows cite the lane record: its own "
        "thirty-four, each ending with a LANE L7 paragraph that records its "
        "class and outcome; and, since the lane's integration (2026-09-24, on "
        "the orchestrator's call), the six intro-editor rows `A8`, `A11`, "
        "`A13`, `A17`, `A19`, `A21`, whose note records that their "
        "COVERED-UNFIRED now rests on the repaired save gate, and `P G3`, "
        "whose ruling the same call named. All 41 sites were re-read on the "
        "merged tree; seventeen have a matched word near, and the words fall in "
        "three groups: (1) `WRONG` in the KEPT text of rows `A14`, `A15` and "
        "`A22` -- *'THE BLOCKER'S PREMISE IS NOW WRONG'*, the census correcting "
        "its OWN earlier blocker in place on 2026-09-19, now also beside the "
        "citing notes on `A13`, `A17` and `A21` -- and the in-place "
        ...
        "assert in public, not about any document. The six intro-editor notes "
        "and `P G3`'s carry no matched word of their own. The census rests "
        "these cells on the lane record and corrects nothing in it. **WHAT "
        "WOULD MAKE THIS ENTRY WRONG:** a profile.md row asserting that "
        "something the lane record says is mistaken while leaving the record "
        "untouched."
    ),
