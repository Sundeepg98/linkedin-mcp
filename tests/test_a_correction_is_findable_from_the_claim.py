...
NOT_A_CORRECTION: dict[tuple[str, str], str] = {
    ...
    ("profile.md", "2026-09-23-live-lane-session-1.md"): (
        "TABLE-ROW PROXIMITY, from TEN citing lines, every matched word in a "
        "NEIGHBOURING row. The session document is cited by rows `A8`, `A11`, "
        "`A13`, `A17`, `A19`, `A21` (each 'NOT SAVED', recording why the "
        "profile-edit proof did not save), `A25`, `L1`, `L8` (each 'CAPTURED', "
        "state unchanged) and `G6` (banked on the session's fire). Read "
        "window by window: `A13`, `A17` and `A21` sit beside rows `A14`, `A15` "
        "and `A22`, whose own paragraph reads 'the blocker's premise is now "
        "wrong: the controls are read' -- about those rows' blockers, written "
        "before this session; and `G6` sits above `G7`, whose 2026-09-05 "
        "paragraph reads '\"no tool, no reason\" is now half false'. No citing "
