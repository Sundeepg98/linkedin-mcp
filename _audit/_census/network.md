...
| 172 | View a fellow group member's connections only after connecting with them | R | COVERED-UNFIRED | **COVERED-UNFIRED 2026-09-24 (lane S): THE `connectionOf` READER OF `N 85`, NEVER FIRED.** A member's connection list is the people search with a `connectionOf` query; `linkedin_people_search_shape(connections_of=...)` composes it in `linkedin_server/people_search.py` (`compose`), the member id from the tool's argument only, never echoed and never stored (`OTHER-MEMBER-IDS-AS-READS`), and the read boundary admits it. The member's own profile stays refused and is not loaded. **SCOPE:** counts, never who -- and for this row the count is the question it asks: a first page that draws person results says that member's connections are shown to this account; a zero does NOT say they are not, because hidden, empty and a dropped facet read alike. **UNFIRED, AND THE FIRE NEEDS A MEMBER HE NAMES:** see the live queue in `_audit/2026-09-24-lane-s-people-search.md`. Guarded by `tests/test_people_search_readers.py`. No prior cell: the row carried none. |
