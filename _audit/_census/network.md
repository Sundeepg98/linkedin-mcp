...
| 4 | Send an invitation from a people-search result | W | GAP | NOT-REV. **THE STATED REASON WAS FALSE AT HEAD AND IS CORRECTED IN PLACE, 2026-09-21. NO STATE MOVES.** The cell read *"Blocker: no people search exists here at all"*. People search has existed since 2026-09-20: `/search/results/people/` is on the read allowlist -- `readonly.is_read_url` True, re-measured 2026-09-21 -- admitted with its shaper and the tool `linkedin_people_search_shape`, and the surface was read live for the first time on 2026-09-21. **THE ROW DOES NOT MOVE, AND THE REASON IT DOES NOT IS STRONGER THAN THE ONE IT CARRIED.** This is the ONE write-direction row on this blocker, and `GROUPS-ADDRESS-BUYS-NO-WRITE` already prices it: an address buys nothing for a write, which needs its own url, its own write sanction and its own ruling. The address third is now paid and the other two are not. It is refused twice over besides: `NO-IRREVERSIBLE-WRITE-IS-FIRED` is standing, and condition 5 of the admitting ruling binds this surface in particular -- *"NOTHING IS FIRED FROM THAT SURFACE. No connect, no follow, no message, no invitation"* -- which names this exact act. **A reader can never move this row**, which is why it is the one row here `scripts/reader_closable_blockers.py` counts as not reader-reachable. See `_audit/2026-09-21-the-search-results-surface.md` **LANE L5, 2026-09-24, R2 CLASSED BLOCKED (a ruling first).** The people-search admission binds that nothing is FIRED from that surface, and a result row's Connect can send an invitation with no dialog. Prerequisite: a ruling on firing from search; then the target from a CALLER-SUPPLIED profile identity (lane S builds the readers). THE CALLER-SUPPLIED ROUTE ON ITS OWN WAS CHECKED AND DOES NOT OPEN IT: acting on a profile he names means loading that member's page, `/in/<slug>/`, which the read boundary refuses (re-measured 2026-09-24), and `writes.PERMANENTLY_FORBIDDEN` already prices a stranger's profile load as an emission that lands on them; `linkedin_send_invitation` aims only at the rail on his own profile. So each route needs a ruling of its own: firing from search, or a named member's profile admitted for an act he orders. |
| 5 | Add a personalized note to an invitation | W | GAP | NOT-REV. `linkedin_send_invitation(member, confirm_token)` takes no note parameter. Nothing in the repo discusses notes **LANE L5, 2026-09-24, R2 CLASSED BLOCKED (on `1`'s first fire).** The note field appears only after Connect is pressed, and pressing Connect can send at once -- so no capture of the field can be made safe; the first supervised `N 1` send at a target he names is the capture. |
| 6 | Re-invite a member after the previous invitation expired | W | GAP | NOT-REV. Requires knowing an invitation expired -- the Sent surface is ruled out **LANE L5, 2026-09-24, R2 CLASSED BLOCKED (the sent-invitations surface).** Knowing an invitation expired needs the Sent manager, whose address carries a forbidden substring. Prerequisite: its admission. |
...
| 166 | Send a connection invitation to a fellow group member from inside the group | W | GAP | NOT-REV. An invitation route that does NOT require opening the person's profile **LANE L5, 2026-09-24, R2 CLASSED BLOCKED (the members address).** `/groups/<id>/members/` is refused, and ruling `GROUPS-ADDRESS-BUYS-NO-WRITE` says an admitted group page buys no write. |
| 167 | Send a message request to a group member you are not connected to | W | GAP | NOT-REV **LANE L5, 2026-09-24, R2 CLASSED BLOCKED (the members address).** `/groups/<id>/members/` is refused, and ruling `GROUPS-ADDRESS-BUYS-NO-WRITE` says an admitted group page buys no write. |
...
| 191 | Message attendees who are already your connections | W | GAP | NOT-REV **LANE L5, 2026-09-24, R2 CLASSED BLOCKED (the attendee list).** `/events/<id>/attendees/` is refused. An attendee he already has a conversation with is reached by `linkedin_send_reply` by thread id -- not claimed here, because this row's capability is reaching them from the event. |
| 192 | Reach attendees who are not your connections via InMail | W | GAP | NOT-REV. The InMail half is EXCLUDED-RULED under R9; the ATTENDEE-TARGETING half is not, and is what makes this a distinct row **LANE L5, 2026-09-24, R2 CLASSED BLOCKED** -- the attendee list as `191`, first-contact addressing as `M M6`; the InMail half's exclusion is the census owner's call. |
...
| A11 | Message an individual group member as owner or manager | W | GAP | needs a second consenting human **LANE L5, 2026-09-24, R2 CLASSED BLOCKED** -- a group or event he administers, its admin surface unadmitted, and a second consenting human; none is on record. |
| A12 | Send a message request as a group admin to a member you are not connected to | W | GAP | needs a second consenting human **LANE L5, 2026-09-24, R2 CLASSED BLOCKED** -- a group or event he administers, its admin surface unadmitted, and a second consenting human; none is on record. |
| A13 | Privately message any event attendee as the organizer, without being connected | W | GAP | needs a second consenting human **LANE L5, 2026-09-24, R2 CLASSED BLOCKED** -- a group or event he administers, its admin surface unadmitted, and a second consenting human; none is on record. |
