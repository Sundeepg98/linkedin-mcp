...
> |---|---|
> | the read allowlist is **22** url patterns | **49**. Twenty of the listed twenty-two survive; **two were deliberately DROPPED** -- the generic `/in/<slug>/` pair went at `956358d` on 2026-09-04, so no pattern today can address a third party's profile by slug at all -- and **29 were added**, the most recent being one event by its numeric id, admitted by a lane-L1 `REVIEW:` commit on 2026-09-23 under the groups-id conditions; before it, his own follower list, admitted past `/follow` by a lane-L1 `REVIEW:` commit on 2026-09-23 (one line and one pattern exemption excusing `/follow` alone); before it, five self-scoped read pages admitted by lane L1 on 2026-09-23 (his contact-info overlay and his articles list in the `/in/me/` form, one post's analytics by activity urn, his audience analytics and his analytics hub -- `_audit/2026-09-23-lane-l1-refused-reads.md`); before them, the `/search/results/people/` admission that landed with a name-free shaper on 2026-09-20. **This cell read 41 for about an hour**, which is the point of deriving it: the guard beside this row caught the drift at the merge that caused it, rather than seventeen days later |
> | nothing in it reaches Groups, Events, LinkedIn Live, newsletters, saved posts, hashtags, article drafts, post analytics, creator analytics, scheduled posts or any media-upload surface | **4 of those 11 are now reachable**: `/groups/`, `/events/`, `/mynetwork/network-manager/newsletters/` and `/analytics/creator/content/` all return True from `is_read_url`. **A FIFTH SINCE 2026-09-23: post analytics**, one post at a time at `/analytics/post-summary/urn:li:activity:<id>/` (lane L1). The other six still refuse, and the substring checks confirm it rather than assume it: `schedul`, `hashtag`, `live` and `video` each match **0** of the 41 patterns |
...
| C48 | View all your articles | a520701 | GAP | R | REV | never named  **MEASURED 2026-09-19: no article read row is one pattern away.** `/in/me/recent-activity/articles/` and `/pulse/` both refuse through `is_read_url` (absolute spellings, controls behaving), so `ARTICLE-SURFACE`'s single `allowlist +1` is owed at least twice -- once for the article list and once for `/pulse/` -- and this wave established no reader behind either. See `_audit/2026-09-19-content-tail.md` section 5 **2026-09-23 (lane L1): THE LIST ADDRESS IS ADMITTED ON A NAMED BASIS; STATE UNCHANGED.** `/in/me/recent-activity/articles/` is on the read allowlist, one anchored line, the `/in/me/` form only. It is the one L1 admission NO capture draws -- his profile draws the parent `/recent-activity/all/` as "Show all posts" -- so it is admitted on the interests / job-alerts precedent: a self-owned address opened once to learn whether LinkedIn serves it. `/pulse/` stays refused. NEEDS-CAPTURE (live-lane key `articles`). See `_audit/2026-09-23-lane-l1-refused-reads.md` |
