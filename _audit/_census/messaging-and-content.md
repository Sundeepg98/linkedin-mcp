...
| C29 | Sort comments (Most relevant / Most recent) | a524166 | GAP | R | REV | never named; a pure view filter, the same class the messaging pills were admitted under **LANE L2, 2026-09-23: STILL NEVER CAPTURED, AND ITS ONLY REACHABLE HOST IS THE FEED.** No comment-sort control is in any of the 25 raw captures or the tracked fixtures. The permalink is admitted but unaddressable -- `C42` is EXCLUDED-RULED because no tool returns a post urn -- and on the feed, comments stay collapsed until a press whose render is unrecorded. Choosing an order is a view switch, not a disclosure, so no ruled press reaches it; the messaging pills are that class, admitted by name on one surface only. A reader would return counts and relations only (`FEED-CONTENT-READ-RULING`). NEEDS-CAPTURE; state unchanged. See `_audit/2026-09-23-lane-l2-refused-presses.md`. |
