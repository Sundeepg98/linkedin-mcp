...
> |---|---|
> | the read allowlist is **22** url patterns | **47**. Twenty of the listed twenty-two survive; **two were deliberately DROPPED** -- the generic `/in/<slug>/` pair went at `956358d` on 2026-09-04, so no pattern today can address a third party's profile by slug at all -- and **27 were added**, the most recent being five self-scoped read pages admitted by lane L1 on 2026-09-23 (his contact-info overlay and his articles list in the `/in/me/` form, one post's analytics by activity urn, his audience analytics and his analytics hub -- `_audit/2026-09-23-lane-l1-refused-reads.md`); before them, the `/search/results/people/` admission that landed with a name-free shaper on 2026-09-20. **This cell read 41 for about an hour**, which is the point of deriving it: the guard beside this row caught the drift at the merge that caused it, rather than seventeen days later |
