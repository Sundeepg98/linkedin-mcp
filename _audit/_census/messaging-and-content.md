...
The same wave added a `HELD BY` citation to `M33` and `M43` (the standing
messaging ruling), which `scripts/census_completion.py` reads to say what holds
each COVERED-UNFIRED row; no state moved there either.
...
| M33 | Apply an inbox filter pill (Focused, Other, Unread, Starred, Connections, Jobs, InMail) | a711117, a542831 | COVERED-UNFIRED | R | REV | `linkedin_open_messaging(message_filter=...)`; closed set of 7 at `dom.py:2269-2276`, the second entry in `readonly.SANCTIONED_MUTATIONS`. Never run live. **HELD BY `DO-NOT-OPEN-MESSAGING`** (2026-09-23): applying a pill means opening `/messaging/`, which lands in a conversation LinkedIn chooses and can mark a real person's message read. What would move this row is the operator lifting or narrowing that ruling, not a session |
...
| M43 | Open the inbox and list conversations | a564261 | COVERED-UNFIRED | R | REV | `linkedin_open_messaging`. Deliberately never called: `_audit/2026-08-30-linkedin-writes.md:335-336` -- "**This wave did NOT call `linkedin_open_messaging`.** The cost lands on somebody who is not him, so it is his to spend." **HELD BY `DO-NOT-OPEN-MESSAGING`** (2026-09-23): that cost is the one the ruling was made on, on 2026-08-31, so what would move this row is the operator lifting or narrowing it, not a session |
