...
The same wave added a `HELD BY` citation to `M33` and `M43`, which
`scripts/census_completion.py` reads to say what holds each COVERED-UNFIRED
row. It first cited the messaging ruling; the operator lifted that ruling at
18:15 the same day, and the citation now names what remains, a live proof
against a target he names. No state moved there either.
...
| M33 | Apply an inbox filter pill (Focused, Other, Unread, Starred, Connections, Jobs, InMail) | a711117, a542831 | COVERED-UNFIRED | R | REV | `linkedin_open_messaging(message_filter=...)`; closed set of 7 at `dom.py:2269-2276`, the second entry in `readonly.SANCTIONED_MUTATIONS`. Never run live. **HELD BY `OPERATOR-NAMES-THE-TARGET`** (2026-09-23): a live proof against a target the operator names. This row was held by `DO-NOT-OPEN-MESSAGING` from 2026-08-31 until the operator lifted it at 18:15 on 2026-09-23 (his ruling (b), relayed by the orchestrator: this server may open messaging). Applying a pill still means opening `/messaging/`, which lands in a conversation LinkedIn chooses and can mark a real person's message read; that is now his to accept, per fire |
...
| M43 | Open the inbox and list conversations | a564261 | COVERED-UNFIRED | R | REV | `linkedin_open_messaging`. Deliberately never called: `_audit/2026-08-30-linkedin-writes.md:335-336` -- "**This wave did NOT call `linkedin_open_messaging`.** The cost lands on somebody who is not him, so it is his to spend." **HELD BY `OPERATOR-NAMES-THE-TARGET`** (2026-09-23): a live proof against a target the operator names. That cost is the one `DO-NOT-OPEN-MESSAGING` was made on, on 2026-08-31, and it held this row until the operator lifted it at 18:15 on 2026-09-23 (his ruling (b), relayed by the orchestrator: this server may open messaging) |
