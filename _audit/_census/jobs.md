...
| 40 | Read per-job network proximity ("2 connections", "1 company alum") | `tests/fixtures/jobs_search_hydrated.html` | GAP `SKILL` | PRESENT AND UNPARSED, which is not the same verdict as absent. Two committed captures carry the field -- `jobs_search_hydrated.html` reads "1 company alum works here" and `job_detail_following_hydrated.html` reads "Company alumni from ..." -- on pages `linkedin_search_jobs` and `linkedin_job_detail` load on an ordinary call, at addresses admitted since the first commit. THE DISCRIMINATOR: the UN-HYDRATED twins of those same two pages read zero on every needle, so the hit is a property of the page and not of a loose regex. No extractor exists anywhere in `linkedin_server/`; the only two mentions are hazard comments about the line shifting `company` into `location`. The skill serves this meanwhile and the row stays GAP. COST one parser, boundary 0. REOPENER a parser over either capture. WHO this repo. |
...
| 127 | Read the InMail credit balance | a7474394 | GAP `SKILL` | **RE-OPENED 2026-09-20.** The MEASURED-ABSENT verdict rested on a reading that could not have produced it: the only instrument that has ever opened `/premium/my-premium/` tallies 16 self-authored needles, 8 entitlement and 8 upsell, and NOT ONE is about a balance, a credit or a number -- `balance`/`credit`/`inmail` are zero hits in the probe, and `read_premium_surface` returns an integer, a boolean, None or one of our needle names BY CONSTRUCTION. It never looked. The `readonly.py` comment states the QUESTION -- whether an InMail balance is a countable thing -- and the cell banked it as the ANSWER. Blocker stands: PREMIUM-READER-NOT-BUILT. |
