...
**FOURTH DELTA, 2026-09-23 (lane L3), and it moves three rows out of GAP at
once.** The block above is still UNCHANGED; this records the movement,
measured by `census_completion.walk()` before and after:

    GAP               56  ->  53    rows 18, 39 and 57, each BUILT
    COVERED-UNFIRED    5  ->   8    same three rows
...
collection's posting ids. `J 57` -- `linkedin_tracked_job_proximity`, the join
row 57's own cell named as the only missing piece. **All three were built
OFFLINE and none has been fired**, so each enters at COVERED-UNFIRED and
moves to COVERED-PROVEN on one live call -- the bar the third delta set for
row 40, applied unchanged. **AND THE 53 THAT REMAIN NOW CARRY A DIRECTION**,
outside this file as section 8 of `_audit/2026-09-21-the-jobs-direction.md`
argued it should be: `_audit/_census/jobs-directions.tsv`, R 25, W 25, R+W 3,
checked on every run by `scripts/check_jobs_directions.py`.
