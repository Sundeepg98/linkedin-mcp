...
| C2 | Customise the public profile URL | W | EXCLUDED-RULED | `/public-profile/settings`; no tool, no reason  **THE `no reason` HALF IS FALSE AND THE ROW MOVES ON THE CORRECTION, 2026-09-20. EXCLUDED-RULED.** The bare word `settings` entered the forbidden substring tuple in `readonly.py` on 2026-09-03, and `/public-profile/settings` is the address that entry's own comment names as the member which exposed the hole: *"A SECOND SPELLING. `/public-profile/settings` has no trailing slash, so `/settings/` -- slashes on both sides, present since the beginning -- does not match it. A gate that turns on a trailing slash is a spelling filter, not a ruling."* That tuple is checked BEFORE the allowlist, so the address cannot be opened however the allowlist is edited later. `C7` on this same table is already EXCLUDED-RULED on exactly this shape (`/psettings/guest-controls`). **CO-LOCATION UNDER A RULED ADDRESS, which is how all 123 of this slice's family-ruling rows are filed -- nobody has weighed this capability on its merits and this row does not claim anybody has.** |
| C3 | Public profile visibility master switch | W | EXCLUDED-RULED | `a528138`: *"Your profile's public visibility"* Off hides it from public view **EXCLUDED-RULED 2026-09-20 BY CO-LOCATION, not a new decision.** `/public-profile/settings` carries the bare `settings` forbidden substring added to `readonly.py` on 2026-09-03 and is named verbatim in that entry's own comment; the section header's *"Nothing else in this repo names it either"* was measured FALSE on 2026-09-20. The full argument is on `C2` and on the corrected section header. |
| C4 | Per-section public visibility toggles | W | EXCLUDED-RULED | `a518980`; the default photo setting is Public **EXCLUDED-RULED 2026-09-20 BY CO-LOCATION, not a new decision.** `/public-profile/settings` carries the bare `settings` forbidden substring added to `readonly.py` on 2026-09-03 and is named verbatim in that entry's own comment; the section header's *"Nothing else in this repo names it either"* was measured FALSE on 2026-09-20. The full argument is on `C2` and on the corrected section header. |
| C5 | Search-engine visibility of the public profile | W | EXCLUDED-RULED | same page **EXCLUDED-RULED 2026-09-20 BY CO-LOCATION, not a new decision.** `/public-profile/settings` carries the bare `settings` forbidden substring added to `readonly.py` on 2026-09-03 and is named verbatim in that entry's own comment; the section header's *"Nothing else in this repo names it either"* was measured FALSE on 2026-09-20. The full argument is on `C2` and on the corrected section header. |
| C6 | Unlink a prior public URL | W | EXCLUDED-RULED | same page **EXCLUDED-RULED 2026-09-20 BY CO-LOCATION, not a new decision.** `/public-profile/settings` carries the bare `settings` forbidden substring added to `readonly.py` on 2026-09-03 and is named verbatim in that entry's own comment; the section header's *"Nothing else in this repo names it either"* was measured FALSE on 2026-09-20. The full argument is on `C2` and on the corrected section header. |
...
| D13 | Licenses and certifications | W | EXCLUDED-RULED | `/edit/` family ruling; and its Help article `a567169` is **HTTP 404 on all three verticals tried**. Its content survives only as a cached search snippet, so its form fields are UNVERIFIED. **REOPENER, NAMED 2026-09-20: the operator admitting this editor's address BY NAME** -- the mechanism that has already let exactly three addresses out of the closed families (dark mode, the intro editor, the skills details page; section 6). WHO: the operator. **AND THE HELP-ARTICLE HALF OF THIS CELL REOPENS NOTHING, flagged because it is the half that LOOKS live:** `a567169` resolving would supply the field names this row records as UNVERIFIED and would not move the row an inch, because the `/edit/` ruling excludes it either way. **A world-fact sitting beside a ruling is colour, not the load-bearing reason** -- and a reader who takes the 404 for the trigger will re-litigate a durable exclusion on a fact that cannot decide it |
| D14 | Courses | W | EXCLUDED-RULED | `/edit/` family ruling; **name-searched, no dedicated Help article exists**. **REOPENER, NAMED 2026-09-20: the operator admitting this section's editor address BY NAME** (section 6 -- three addresses have ever left these families, each on an operator ruling). WHO: the operator. **THIS REOPENER IS INHERITED BY `D15`, `D16` AND `D17`**, whose cells read `same` and resolve here by backreference, and it is worded to read correctly from any of the four. **The Help-index half reopens nothing:** a dedicated article appearing would document the section and would not touch the ruling that excludes it |
...
| # | capability | R/W | state | evidence / blocker |
|---|---|---|---|---|
| K1 | Verifications section on the profile | R | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
| K2 | Identity verification via DigiLocker (India -- his route) | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
| K3 | Workplace verification by work email | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
| K4 | Workplace verification via Microsoft Entra Verified ID | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
| K5 | Workplace verification via a company LinkedIn Learning licence | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
| K6 | Phone number verification | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
...
| M8 | "Share resume data with recruiters" toggle | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
| M9 | Stored job applicant accounts | R/W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) (`/mypreferences/d/job-application-accounts`); denylist-uncaught, finding 7.1 |
| M10 | Autofill for work emails | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
...
| N6 | Passkeys add / delete | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10). `a1621596`: max 5 per account |
| N7 | Email addresses -- add / remove / change primary | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10), `/psettings/email` |
| N8 | Phone numbers -- add / remove / choose reset number | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
| N9 | Where you're signed in -- list active sessions | R | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10), `/psettings/sessions`. Shows location, IP, device, browser, last sign-in |
| N10 | Sign out of one session / all sessions | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10), same page. A distinct act from reading the list |
| N11 | Devices that remember your password | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
| N12 | Keep me logged in / auto sign-in | W | EXCLUDED-RULED | `/uas/login` -- not a settings-index page, caught by no substring, named nowhere in this repo. `a1342645`: unavailable when 2FA is on  **THE `caught by no substring` CLAUSE IS FALSE AND THE ROW MOVES ON THE CORRECTION, 2026-09-20. EXCLUDED-RULED.** `/uas/` entered the forbidden substring tuple in `readonly.py` on 2026-09-03 with its reason written beside it -- *"A LEGACY NAMESPACE. `/uas/` is LinkedIn's old auth tree and the exact sibling of the `/psettings/` three lines up, which is on this list for precisely the stated reason that a legacy address is one LinkedIn can start serving again."* That tuple is checked BEFORE the allowlist. `password` and `two-factor` are on it too and reach the same family. **CO-LOCATION UNDER A RULED ADDRESS, not a judgement that auto sign-in was weighed and declined.** |
...
| N15 | Interface language | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10). `a521833` names ONE language control |
| N16 | Autoplay videos | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
| N17 | Sound effects | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
| N18 | Showing other members' profile photos | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
| N19 | Name / location / industry via settings | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10); also reachable through the intro editor (block A) |
| N20 | Calendar sync | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10); mobile app only **REOPENER, NAMED 2026-09-20: the operator admitting this setting BY NAME**, which is the settings-family mechanism and is how dark mode got in -- section 6, and `network.md` **R11**. WHO: the operator. **THE `mobile app only` CLAUSE IS THE CONTINGENT HALF AND IT IS NOT THE LOAD-BEARING ONE, flagged so the two are not confused:** a calendar-sync control appearing on the DESKTOP settings surface would falsify that clause and would leave the row exactly where it is, because the settings family is admitted by name or not at all. WHO for that half: an enumeration of the settings index, which this repo already runs |
| N21 | Contact / address-book sync | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
| N22 | Subscriptions and payments | R | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
| N23 | Partners and services / Microsoft account link | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
| N24 | Permitted Services -- third-party app access | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10), `/psettings/permitted-services` |
| N25 | LinkedIn Services (DMA) connect / disconnect | W | EXCLUDED-RULED | `a6222119`: Jobs, Marketing Solutions, Learning. Nav path only -- no URL, so no address to rule on. **RE-FILED 2026-09-19, `NO-URL-AT-ALL`. NOT a new decision -- the operator's settings ruling already covers it**, and the refusal needs no measurement of LinkedIn at all. Refused from two written directions: `/psettings/` and `/settings/` are on `_FORBIDDEN_URL_SUBSTRINGS`, checked BEFORE the allowlist, and DMA data-sharing controls live under that tree; and `linkedin_server/server.py:7364,7401-7402` -- "the read allowlist admits exactly one page below the settings index, admitted BY NAME on the operator's ruling that a setting is admitted by name or not at all." Same ruling already carries `M37`, `M41`, `M42`, `M46`, `M50`. REOPENER: the operator naming this setting -- that is the mechanism, not a consolation; it is how dark mode got in. **NOTE, and it is load-bearing: five rows cite this ruling as `server.py`, in `linkedin_update_setting` (7401-7402 at 2026-09-19 -- **cite the SYMBOL; 6476-6484 is now `_write_tool`**). That range is now `linkedin_save_job`.** The ruling moved ~900 lines and nothing detected it |
| N26 | Mentions, tags and collaborations permission | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10), `a522861` |
| N27 | Merge or close duplicate accounts | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
...
| O21 | Block / unblock a member | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10); independently an act on another member's relationship |
| O22 | See a list of who you blocked | R | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) |
