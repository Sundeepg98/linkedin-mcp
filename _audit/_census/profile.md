...
| D25 | "Add profile section" menu contents | R | MEASURED-ABSENT | The nineteen items below were enumerated from `a540837` -- LinkedIn's HELP ARTICLE, never the page. **READ LIVE 2026-09-19 and the control is not drawn**: on a 235-control own-profile render, `add_profile_section` = 0 under both accessible-name spellings. The detector passed 10 of 10 on owned markup, AND the page control fired IN THE SAME TOP CARD -- `edit` 6, `open_to` 5, `hiring` 3 -- so the card rendered and the reader could see controls inside it. Corroborated by a second instrument on a separate load reading the same 235 controls. NOT a below-the-fold zero. **UNSETTLED: whether LinkedIn draws it under a different name** -- 9 controls carry `aria-haspopup` and `dom.read_surface_census` discards raw accessible names by design, so naming them needs a shaped enumeration. Prior enumeration kept for the record: **Core** About, Education, Position, Services, Career break, Skills; **Recommended** Featured, Licenses and certifications, Projects, Courses, Recommendations; **Additional** Volunteer experience, Publications, Patents, Honors and awards, Test scores, Languages, Organizations, Causes. Evidence `_audit/_scratch/_probe-profile-modal-presence.txt` |
