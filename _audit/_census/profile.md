...
| A8 | Additional name | W | COVERED-UNFIRED | **NOT SAVED 2026-09-23 22:46 (live lane, session 1); ZERO WRITES.** The profile-edit ruling's first condition is notify-network CONFIRMED OFF IN THE EDIT DIALOG. The shipped `linkedin_profile_editor_fields` read 17 controls in the intro editor and none is a notify control: the only two switches are 'Open Profile' and 'Profile Premium Badge' (identified offline from the capture). A condition that cannot be proven means no save, by the brief's rule. What would make it provable: a reading of the account-level share-profile-updates setting plus a ruling that it satisfies condition 1, and the write path asserting it at save time. See `_audit/2026-09-23-live-lane-session-1.md` Entry 5. aimable by `label-for`; the gate would not refuse it; never fired |
...
| A11 | Country / Region | W | COVERED-UNFIRED | **NOT SAVED 2026-09-23 22:46 (live lane, session 1); ZERO WRITES.** The profile-edit ruling's first condition is notify-network CONFIRMED OFF IN THE EDIT DIALOG. The shipped `linkedin_profile_editor_fields` read 17 controls in the intro editor and none is a notify control: the only two switches are 'Open Profile' and 'Profile Premium Badge' (identified offline from the capture). A condition that cannot be proven means no save, by the brief's rule. What would make it provable: a reading of the account-level share-profile-updates setting plus a ruling that it satisfies condition 1, and the write path asserting it at save time. See `_audit/2026-09-23-live-lane-session-1.md` Entry 5. aimable by `aria-label` |
