...
| D5 | Add / edit / delete a position | W | EXCLUDED-RULED | `/edit/` family ruling. `a593695` covers jobs, volunteering, military, board service; it does not enumerate field names **DRAWN, 2026-09-24 (lane Y2 completeness admission):** the add-an-Experience form `/in/<me>/edit/forms/position/new/`. |
...
| G4 | Featured: reorder / edit / remove / unpin | W | EXCLUDED-RULED | `/edit/` family ruling, plus `drag_to` **DRAWN, 2026-09-24 (lane Y2 completeness admission):** the section's control 'Featured overflow menu'. |
...
| I12 | Job preferences / career-interests page | W | EXCLUDED-RULED | measured: zero of 237 urls reach one **DRAWN, 2026-09-24 (lane Y2 completeness admission):** `/jobs/preferences/`, the jobs nav's 'Preferences' link (captured 2026-09-20) -- a drawn address for the page this cell measured zero urls reaching. |
...
| # | capability | R/W | state | evidence / blocker |
|---|---|---|---|---|
| K1 | Verifications section on the profile | R | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) **DRAWN, 2026-09-24 (lane Y2 re-harvest):** the settings index draws the section as a page, `/mypreferences/d/verifications/` ('Verifications', captured 2026-09-23). |
| K2 | Identity verification via DigiLocker (India -- his route) | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) **DRAWN, 2026-09-24 (lane Y2 completeness admission):** 'Verify now' on the Premium hub opens `/trust/verification/`. |
...
| N15 | Interface language | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10). `a521833` names ONE language control **DRAWN, 2026-09-24 (lane Y2 re-harvest):** `/mypreferences/d/settings/language/` ('Language' on the settings index, captured 2026-09-23). |
| N16 | Autoplay videos | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) **DRAWN, 2026-09-24 (lane Y2 re-harvest):** `/mypreferences/d/settings/autoplay-videos/` ('Autoplay videos' on the settings index, captured 2026-09-23). |
| N17 | Sound effects | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) **DRAWN, 2026-09-24 (lane Y2 re-harvest):** `/mypreferences/d/settings/enable-sounds-desktop/` ('Sound effects' on the settings index, captured 2026-09-23). |
| N18 | Showing other members' profile photos | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) **DRAWN, 2026-09-24 (lane Y2 re-harvest):** `/mypreferences/d/settings/show-profile-photos/` ('Showing profile photos' on the settings index, captured 2026-09-23). |
| N19 | Name / location / industry via settings | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10); also reachable through the intro editor (block A) **DRAWN, 2026-09-24 (lane Y2 re-harvest):** `/profile/<id>/` ('Name, location, and industry' on the settings index, captured 2026-09-23). |
| N20 | Calendar sync | W | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10); mobile app only **REOPENER, NAMED 2026-09-20: the operator admitting this setting BY NAME**, which is the settings-family mechanism and is how dark mode got in -- section 6, and `network.md` **R11**. WHO: the operator. **THE `mobile app only` CLAUSE IS THE CONTINGENT HALF AND IT IS NOT THE LOAD-BEARING ONE, flagged so the two are not confused:** a calendar-sync control appearing on the DESKTOP settings surface would falsify that clause and would leave the row exactly where it is, because the settings family is admitted by name or not at all. WHO for that half: an enumeration of the settings index, which this repo already runs **DRAWN, 2026-09-24 (lane Y2 re-harvest):** `/mynetwork/settings/manage-syncing/`, drawn as both 'Sync calendar' and 'Sync contacts' (N21) on the settings index (captured 2026-09-23). |
...
| N22 | Subscriptions and payments | R | EXCLUDED-RULED | settings family -- ruling: `linkedin_update_setting` (`linkedin_server/server.py`), *"a setting is admitted by name or not at all"*; the SAME ruling as `network.md` **R11** and messaging **MESSAGING-SETTINGS** (3.10) **DRAWN, 2026-09-24 (lane Y2 completeness admission):** the purchase's own manage link at `/manage/purchases-payments/purchases/<id>/`; changing the plan is row S9. **DRAWN (lane Y2 re-harvest):** 'View purchase history' at `/manage/purchases-payments/purchases/` and 'Manage Premium account' at `/mypreferences/d/premium-manage-account/` (captured 2026-09-23). |
...
| N28 | Close (delete) account | W | EXCLUDED-RULED | `"/close-accounts"` was added to the forbidden tuple 2026-08-31 precisely because nothing else caught it, plus `delete_or_withdraw_anything` **DRAWN, 2026-09-24 (lane Y2 re-harvest):** `/mypreferences/d/close-accounts/` ('Close and delete account' on the settings index, captured 2026-09-23). |
| N29 | Hibernate account | W | EXCLUDED-RULED | `"/hibernate-account"`, added the same day, same reason **DRAWN, 2026-09-24 (lane Y2 re-harvest):** `/mypreferences/d/hibernate-account/` ('Hibernate account' on the settings index, captured 2026-09-23). |
