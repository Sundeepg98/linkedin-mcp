"""The tool surface: fifty-five tools, fourteen of which write to LinkedIn.

THE FIFTY-THIRD TO FIFTY-FIFTH ARRIVED TOGETHER, 2026-09-24, at the merge
...
``M M17`` and ``M M49``, COVERED-UNFIRED. The headline read "fifty-two tools,
...
than about an intention. Fifty-five is ``len(await mcp.list_tools())``,
pinned in ``test_server_surface.py`` by
``test_the_surface_is_exactly_the_fifty_five_tools``; the split is pinned by
...
needs: FORTY-ONE read, FOURTEEN write, and ZERO are write-shaped,
registered, gated and unable to act. Forty-one plus fourteen plus zero is
fifty-five. (Thirty-eight, thirteen and fifty-one until 2026-09-24; thirty-nine,
thirteen and fifty-two at the live lane's merge the same day.)
    ...
    try:
        full = {
            ...
            "known_side_effects": [
                ...
                "and linkedin_newsletter_subscriptions -- and neither "
                "assumes the cost in either direction: each reads the "
                "pending-invitation badge off the feed BEFORE and off the "
                "loaded page AFTER, and REFUSES to return a result if "
                "either reading fails or the badge moved. A load that "
                "cannot certify its own cost does not hand back a clean "
                "answer. **THIS SENTENCE READ 'no tool here loads it at "
                "all' UNTIL 2026-09-05 EVENING AND WAS ALREADY WRONG WHEN "
                "IT WAS WRITTEN**: linkedin_connections shipped on "
                "2026-09-03 loading /mynetwork/invite-connect/connections/ "
                "with exactly that discipline, and read charitably the "
                "sentence meant the root while a caller would read it as "
                "the whole surface. Corrected when a second tool joined it, "
                "by the wave that added the second. "
                "linkedin_send_invitation still opens nothing here: it "
                "reads the invitation controls on your own profile, a "
                "surface carrying no such counter.",
